...
NOT_FOUND_MSG = "لم أجد إجابة لهذا السؤال في المستندات المتوفرة."
...
4. لا تقل «{NOT_FOUND_MSG}» إلا إذا كانت كل المقاطع خارج موضوع السؤال تمامًا، وعندها فقط أضف «اقتراح عام (ليس من مستنداتك):» إن كان لديك ما يفيد.
...
def select_hits(col, question: str, cfg):
    """
    ...
    pool = _query(col, q_vec, min(k * 3, 24))
